...
export const sendRegisterConfirm = (data: AuthData) => {
  return async (dispatch: AppDispatch) => {
    ...
    const response = await axiosPost<AuthError | DbError>('/auth/confirm_register', data)
    response
      ...
      .mapLeft(e => {
        if (e.response?.data.name === 'AuthError') {
...
export const loginUser = (data: AuthData) => async (dispatch: AppDispatch) => {
  dispatch(setAuthLoading(true))
  const response = await axiosPost<AuthError | DbError, User>('/auth/login', data)
  response
    .mapRight(({ data: user }) => {
...
export const authByProvider = (provider: ProviderType, providerName: AuthProviderName) => {
  return async (dispatch: AppDispatch) => {
    ...
    googleResponse
      ...
      .mapRight(async (data) => {
        const userResponse = await axiosPost<DbError, User, AuthByProviderBody>('/auth/provider', { ...data })
        userResponse
          .mapRight(({ data: user }) => {
            localStorage.setItem('token', user.token)
            dispatch(setUser(user))
